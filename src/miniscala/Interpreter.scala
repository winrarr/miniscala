...
object Interpreter {
  ...
  def eval(e: Exp, env: Env, cenv: ClassEnv, sto: Sto): (Val, Sto) = e match {
    ...
    case NullLit() => (RefVal(-1, Some(NullType())), sto)
    case VarExp(x) =>
    ...
    case LookupExp(objexp, member) =>
      ...
      objval match {
        case RefVal(loc, _) =>
          if (loc == -1) {
            throw new InterpreterError(s"Unable to look up '$member' from null object '${unparse(objexp)}''", e)
          }
          sto1(loc) match {
  ...
  def checkValueType(v: Val, ot: Option[Type], n: AstNode): Unit = ot match {
    case Some(t) =>
      (v, t) match {
        ...
        case (RefVal(-1, _), _: DynamicClassType) => // do nothing
        case _ =>
